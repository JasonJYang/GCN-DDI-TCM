...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (drug1, drug2, target) in enumerate(self.data_loader):
            if batch_idx == 0:
                gcn = True
            else:
                gcn = False
            drug1 = drug1.to(self.device)
            drug2 = drug2.to(self.device)
            target = target.to(self.device)

            self.optimizer.zero_grad()
            output = self.model(drug1, drug2, gcn)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (drug1, drug2, target) in enumerate(self.valid_data_loader):
                drug1 = drug1.to(self.device)
                drug2 = drug2.to(self.device)
                target = target.to(self.device)

                output = self.model(drug1, drug2)
                loss = self.criterion(output, target)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_idx, (drug1, drug2, target) in enumerate(self.test_data_loader):
                drug1 = drug1.to(self.device)
                drug2 = drug2.to(self.device)
                target = target.to(self.device)
